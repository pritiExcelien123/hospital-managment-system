...
@section('content_title',__('Medicine Registration'))

@section('content_description',__("Register Medicine"))
@section('breadcrumbs')
...
<div @if (session()->has('regpsuccess') || session()->has('regpfail')) style="margin-bottom:0;margin-top:3vh" @else
    ...
    <div class="col-md-10">
        ...
        <div class="alert alert-success alert-dismissible">
            ...
            <button
                onclick="window.open('{{route('medicine',session()->get('pid'))}}','myWin','scrollbars=yes,width=830,height=500,location=no').focus();"
    ...
    <div class="col-md-10">
        ...
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">{{__('Medicine Registration Form')}}</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form method="post" action="{{ route('register-medicine') }}" class="form-horizontal">
                {{csrf_field()}}
                <div class="box-body">
                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">{{__('Medicine Name')}} <span
                                style="color:red">*</span></label>
                        <div class="col-sm-10">
                            <input type="text"  pattern="^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$"
                                 required class="form-control" name="medicine_name" placeholder="Enter Medicine Full Name">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">{{__('Company Name')}} <span
                                style="color:red">*</span></label>
                        <div class="col-sm-10">
                            <input type="text" 
                                 required class="form-control" name="company_name" placeholder="Enter Medicine Company Name">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">{{__('Expiry Date')}} <span
                                style="color:red">*</span></label>
                        <div class="col-sm-2">
                            <input type="date" 
                                 required class="form-control" name="expiry_date" placeholder="Enter Medicine Expiry Date">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-2 control-label">{{__('Quantity')}} <span
                                style="color:red">*</span></label>
                        <div class="col-sm-10">
                            <input type="text" 
                                 required class="form-control" name="quantity" placeholder="Enter Medicine Quantity">
                        </div>
                    </div>
